...
from utils.error_handler import setup_logging, handle_error, log_operation
from utils.config import CONFIG, get_config
from utils.db_utils import test_connection, get_db_session, close_db_session
# Modifica l'importazione per utilizzare le funzioni disponibili
from db.funnel_operations import get_products, get_funnel_by_product_id
...
log_operation("Avvio applicazione", {
...
try:
    if not test_connection():
        st.error("🚨 Impossibile connettersi al database. Verifica le credenziali e la connessione.")
        st.stop()
...
def main():
    ...
    # Footer
    st.caption(f"© 2025 YourCompany Inc. - Ambiente: {os.environ.get('APP_ENV', 'development').upper()}")
