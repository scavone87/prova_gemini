from sqlalchemy import Column, Integer, String, Boolean, ForeignKey, JSON, TIMESTAMP, Float, BigInteger, Text, Date, Numeric, create_engine, text
...
def init_connection():
    ...
    try:
        ...
        logger.error(f"Errore durante l'inizializzazione della connessione al database: {str(e)}")
        return None
...
class Product(Base):
    ...
    product_image_id = Column(Integer, ForeignKey('product.product_images.id'), nullable=False)
...
class Step(Base):
    ...
    # Relationships
    from_routes = relationship('Route', foreign_keys='Route.fromstep_id', back_populates='from_step')
    to_routes = relationship('Route', foreign_keys='Route.nextstep_id', back_populates='next_step')
...
class Route(Base):
    ...
    # Relationships
    next_step = relationship('Step', foreign_keys=[nextstep_id], back_populates='to_routes')
    from_step = relationship('Step', foreign_keys=[fromstep_id], back_populates='from_routes')
...
class Structure(Base):
    ...
    # Relationships
    structure_component_sections = relationship('StructureComponentSection', back_populates='structure')
...
class ComponentSection(Base):
    ...
    structure_component_sections = relationship('StructureComponentSection', back_populates='component_section')
...
class StructureComponentSection(Base):
    ...
    id = Column(BigInteger, primary_key=True)
    component_sectionid = Column(BigInteger, ForeignKey('design.component_section.id'), nullable=False)
    structureid = Column(BigInteger, ForeignKey('design.structure.id'), nullable=False)
    order = Column(Integer, nullable=False)
    
    # Relationships
    component_section = relationship('ComponentSection', back_populates='structure_component_sections')
...
class CmsKey(Base):
    ...
    structurecomponentsectionid = Column(BigInteger, ForeignKey('design.structure_component_section.id'), nullable=False)
    
    # Relationships
    structure_component_section = relationship('StructureComponentSection', back_populates='cms_keys')