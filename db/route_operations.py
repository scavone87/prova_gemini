...
def get_routes_for_workflow(workflow_id):
    ...
    try:
        ...
        routes = session.execute(
            select(
                Route.id, Route.workflow_id, Route.fromstep_id, Route.nextstep_id,
                Route.route_config,
            ...
            .join(next_step_alias, next_step_alias.id == Route.nextstep_id, isouter=True)
        ...
        logger.error(f"Errore nel recupero delle route per il workflow {workflow_id}: {e}")
        return None
    finally:
        close_db_session(session)
...
def delete_route(route_id):
    ...
    try:
        ...
        if not route:
            return {
                'error': True,
                'message': f"La route con ID {route_id} non esiste"
            }
        ...
        return {
            'error': False,
            'message': f"Route eliminata con successo"
        }
    except SQLAlchemyError as e:
        session.rollback()
