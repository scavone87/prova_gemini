...
CMS_KEY_TEMPLATES = {
    "text": {"it": "", "en": ""},
    "image": {"url": "", "alt": {"it": "", "en": ""}},
    "link": {"url": "", "text": {"it": "", "en": ""}, "target": "_blank"},
    "button": {"url": "", "label": {"it": "", "en": ""}, "style": "primary"},
}
...
def cms_key_form(
    ...
    with st.expander("CMS Key Editor", expanded=True):
        ...
        selected_template = st.selectbox(
            ...
            template_options,
            index=(
                template_options.index(st.session_state[template_key])
                if st.session_state[template_key] in template_options
                else len(template_options) - 1
            ),
        ...
        with st.form(key=form_key):
            ...
            with st.expander("Visualizza JSON", expanded=False):
                st.code(
                    json.dumps(st.session_state[data_key], indent=2), language="json"
                )
...
def _render_image_form(data: Dict):
    ...
    if isinstance(alt, dict):
        alt["it"] = st.text_input(
            "Testo alternativo (Italiano):", value=alt.get("it", "")
        )
        alt["en"] = st.text_input(
            "Testo alternativo (Inglese):", value=alt.get("en", "")
        )
    else:
...
def _render_link_form(data: Dict):
    ...
    data["target"] = st.selectbox(
        ...
        index=["_self", "_blank", "_parent", "_top"].index(
            data.get("target", "_blank")
        ),
    )
...
def _render_button_form(data: Dict):
    ...
    if isinstance(label, dict):
        label["it"] = st.text_input(
            "Etichetta pulsante (Italiano):", value=label.get("it", "")
        )
        label["en"] = st.text_input(
            "Etichetta pulsante (Inglese):", value=label.get("en", "")
        )
    else:
    ...
    data["style"] = st.selectbox(
        ...
        index=["primary", "secondary", "outline", "link"].index(
            data.get("style", "primary")
        ),
    )
...
def _render_custom_form(data: Dict):
    ...
    json_str = st.text_area(
        "JSON CMS Key:", value=json.dumps(data, indent=2), height=300
    )
    try:
