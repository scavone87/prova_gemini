...
from db.models import Product, Funnel, Workflow, Step, Route
    ...
    try:
        with st.spinner("Caricamento delle statistiche del sistema..."):
            ...
            if funnels_count > 0:
                ...
                avg_steps_per_funnel = optimize_query_execution(
            ...
            if funnels_count > 0:
                ...
                avg_routes_per_funnel = optimize_query_execution(
    ...
    try:
        with st.spinner("Analisi dei dati del funnel in corso..."):
            ...
            if funnel_id:
                ...
                ).bindparams(funnel_id=int(funnel_id)).fetchone()
                ...
                if steps:
                    ...
                    if len(steps_list) > 1:
                        overall_conversion = steps_list[-1]['users'] / steps_list[0]['users']
                    else:
                ...
                # Formatta i risultati
                return {
                    'top_funnels': [{'id': f.id, 'name': f.name, 'product_name': f.product_name, 'step_count': f.step_count} for f in top_funnels],
                    'funnel_distribution': [{'step_count': d.step_count, 'funnel_count': d.funnel_count} for d in funnel_distribution],
                    'product_distribution': [{'title_prod': d.title_prod, 'funnel_count': d.funnel_count} for d in product_distribution]
                }
    except Exception as e:
...
selected_funnel_id = funnel_options[selected_funnel_idx]['id'] if selected_funnel_idx > 0 else None
...
if selected_funnel_id:
    ...
    if funnel_details and not funnel_details.get('error') and 'funnel' in funnel_details:
        ...
        with metrics_col3:
            st.metric("Conversione Complessiva", f"{funnel_details.get('overall_conversion', 0):.1%}")
    else:
        st.warning(f"Non sono disponibili dati dettagliati per il funnel selezionato. {funnel_details.get('message', 'Dati non disponibili')}")
else:
    ...
    if funnel_stats:
        ...
        if funnel_stats.get('top_funnels'):
            ...
            top_funnel_chart = alt.Chart(df_top_funnels).mark_bar().encode(
        ...
        if funnel_stats.get('funnel_distribution'):
            ...
            with col1:
                ...
                distribution_chart = alt.Chart(df_distribution).mark_arc().encode(
                    theta=alt.Theta('funnel_count:Q', stack=True),
                    color=alt.Color('step_count:N', scale=alt.Scale(scheme='blues')),
                    tooltip=[
                        alt.Tooltip('step_count:N', title='Numero di Step'),
                        alt.Tooltip('funnel_count:Q', title='Numero di Funnel')
                    ]
                ).properties(
                    title='Distribuzione dei Funnel per Numero di Step',
                    height=400
                )
            ...
            if funnel_stats.get('product_distribution'):
                with col2:
                    ...
                    product_chart = alt.Chart(df_product_dist).mark_bar().encode(
                        y=alt.Y('title_prod:N', title='Prodotto', sort='-x'),
                        x=alt.X('funnel_count:Q', title='Numero di Funnel'),
                        color=alt.Color('funnel_count:Q', scale=alt.Scale(scheme='viridis')),
                        tooltip=[
                            alt.Tooltip('title_prod:N', title='Prodotto'),
                            alt.Tooltip('funnel_count:Q', title='Numero di Funnel')
                        ]
                    ).properties(
                        title='Distribuzione dei Funnel per Prodotto',
                        height=400
                    )
...
st.sidebar.caption("I dati vengono aggiornati automaticamente ogni 30 minuti. Ultimo aggiornamento: " + 
