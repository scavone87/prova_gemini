...
import json
import copy
from db.step_operations import get_steps, get_steps_for_workflow, create_step, update_step, delete_step
...
def cached_get_steps():
    ...
    with st.spinner("Caricamento degli step..."):
        try:
            ...
            return handle_error(e, "Errore durante il recupero degli step", fallback_data=[]).get('data', [])
...
def cached_get_steps_for_workflow(workflow_id):
    ...
    with st.spinner(f"Caricamento degli step del workflow {workflow_id}..."):
        try:
            ...
            log_operation("Recupero step per workflow", {"workflow_id": workflow_id, "count": len(steps)})
            return steps
        except Exception as e:
            return handle_error(
                e, 
...
def save_state_for_undo(action_type, step_data):
    ...
    st.session_state.undo_stack.append({
        ...
        'step_data': copy.deepcopy(step_data)
    })
    
    logger.debug(f"Stato salvato per annullamento: {action_type} su step {step_data.get('id')}")
...
def handle_step_update(step_id, updated_data):
    ...
    original_step = next((s for s in cached_get_steps_for_workflow(st.session_state.workflow_id) if s['id'] == step_id), None)
...
def handle_step_delete(step_id):
    ...
    step_to_delete = next((s for s in cached_get_steps_for_workflow(st.session_state.workflow_id) if s['id'] == step_id), None)
...
if 'invalidate_step_cache' in st.session_state and st.session_state.invalidate_step_cache:
...
if 'undo_stack' in st.session_state and st.session_state.undo_stack:
    st.sidebar.button("↩️ Annulla ultima azione", on_click=handle_undo_action, type="secondary")
...
with col2:
    ...
    if workflow_steps:
        ...
        if len(df_view.columns) > 0:
            ...
            columns_to_display = [col for col in columns_to_display if col in df_view.columns]
            
            # Configura le colonne del dataframe
            column_config = {
                "id": st.column_config.NumberColumn("ID", help="Identificativo dello step"),
                "step_url": st.column_config.TextColumn("URL", help="URL dello step"),
                "step_code": st.column_config.TextColumn("Codice", help="Codice identificativo"),
                "post_message": st.column_config.CheckboxColumn("Post Message", help="Post message abilitato")
            }
        ...
        selected_step_id = st.selectbox(
            ...
            format_func=lambda x: "Seleziona uno step..." if x is None else f"Step {x} - {next((s['step_url'] for s in workflow_steps if s['id'] == x), '')}"
        )
        ...
        if selected_step_id:
            ...
            selected_step = next((s for s in workflow_steps if s['id'] == selected_step_id), None)
            if selected_step:
                ...
                with st.container(border=True):
                    ...
                    with col1:
                        st.write("**Post message:**", "✅ Abilitato" if selected_step.get('post_message') else "❌ Disabilitato")
                    ...
                    with col1:
                        ...
                        if st.session_state.confirm_delete:
                            st.warning("Sei sicuro di voler eliminare questo step? L'operazione può essere annullata dopo.")
                    ...
                    with col2:
                        ...
                        if st.button("✏️ Modifica"):
                            ...
                            st.session_state.edit_step_code = selected_step.get('step_code', '')
                            st.session_state.edit_post_message = selected_step.get('post_message', False)
                            st.session_state.edit_shopping_cart = json.dumps(selected_step.get('shopping_cart', {}), indent=2) if selected_step.get('shopping_cart') else ''
                            st.session_state.edit_gtm_reference = json.dumps(selected_step.get('gtm_reference', {}), indent=2) if selected_step.get('gtm_reference') else ''
                
                # Form di modifica, mostrato solo quando necessario
                if 'editing_step' in st.session_state and st.session_state.editing_step and st.session_state.edit_step_id == selected_step_id:
                    with st.form("edit_step_form"):
                        st.subheader("Modifica Step")
                        
                        edit_url = st.text_input("URL dello Step:", value=st.session_state.edit_step_url)
                        edit_step_code = st.text_input("Step Code:", value=st.session_state.edit_step_code)
                        edit_post_message = st.checkbox("Post Message", value=st.session_state.edit_post_message)
                        edit_shopping_cart = st.text_area("Shopping Cart (JSON):", value=st.session_state.edit_shopping_cart, height=100)
                        edit_gtm_reference = st.text_area("GTM Reference (JSON):", value=st.session_state.edit_gtm_reference, height=100)
                        ...
                        with col1:
                            if st.form_submit_button("Salva Modifiche", type="primary"):
                                ...
                                if shopping_cart == "error":
                                    st.error("Il formato JSON di shopping_cart non è valido")
                                    st.stop()
                                
                                gtm_reference = validate_json_input(edit_gtm_reference)
                                if gtm_reference == "error":
                                    st.error("Il formato JSON di gtm_reference non è valido")
                                    st.stop()
        ...
        st.info("Nessuno step associato a questo workflow. Crea nuovi step utilizzando il form a sinistra.")
    ...
    with st.expander("Altri Step Disponibili"):
        ...
        if all_steps:
            ...
            all_steps_pages = (len(all_steps) + all_steps_per_page - 1) // all_steps_per_page
            ...
            if all_steps_pages > 1:
                ...
                with col_info:
                    st.info(f"Visualizzazione pagina {all_current_page} di {all_steps_pages}")
            ...
            if len(df_all_steps.columns) > 0:
                ...
                # Assicurati che tutte le colonne esistano nel DataFrame
                columns_to_display = [col for col in columns_to_display if col in df_all_steps.columns]
...
with st.sidebar:
    ...
    preview_mode = st.checkbox("Modalità Anteprima", 
                              help="Attiva per visualizzare il funnel come lo vedrebbe un utente finale")
    if preview_mode:
        st.info("Modalità anteprima attiva. Le funzionalità di modifica sono disabilitate.")
