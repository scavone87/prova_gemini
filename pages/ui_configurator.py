...
def load_sections_for_step(step_id):
    ...
    product_id = st.session_state.selected_product_id if 'selected_product_id' in st.session_state else None
...
def add_section_to_step():
    ...
    if (st.session_state.current_step_id and 
...
def add_component_to_section():
    ...
    if ('selected_section_id' in st.session_state and 
        'selected_component' in st.session_state and 
        st.session_state.selected_section_id and 
        st.session_state.selected_component):
        
        # Trova l'ultimo ordine esistente e aggiungi 1
        existing_components = load_components_for_section(st.session_state.selected_section_id)
        next_order = 1
...
def update_component_order(component_section_id, new_order):
    ...
    result = ui_operations.update_component_section_order(component_section_id, new_order)
    if not result['error']:
        st.session_state.notification = {
            'type': 'success',
            'message': result['message']
...
def save_cms_key(structure_component_section_id, cms_data):
    ...
    result = ui_operations.create_or_update_cms_key(structure_component_section_id, cms_data)
    if not result['error']:
        st.session_state.notification = {
            'type': 'success',
            'message': result['message']
...
with col2:
    ...
    with tab1:
        ...
        if st.session_state.current_step_id:
            ...
            with st.form("layout_form"):
                st.selectbox(
                    "Tipo di layout:",
                    options=["Singola colonna", "Due colonne", "Tre colonne", "Sidebar"],
                    key="ui_layout_type"
                )
                
                st.slider("Larghezza contenuto (%)", 30, 100, 70, key="ui_content_width")
    ...
    with tab2:
        ...
        if st.session_state.current_step_id:
            ...
            with st.expander("Aggiungi nuova sezione"):
                st.text_input("Tipo di sezione", key="new_section_type", 
            ...
            st.selectbox("Seleziona una sezione da aggiungere:", 
                         options=section_options,
                         key="selected_section_display")
            
            # Mappa il nome della sezione selezionata all'ID
            if 'selected_section_display' in st.session_state and st.session_state.selected_section_display:
                try:
                    index = section_options.index(st.session_state.selected_section_display)
                ...
                for section in step_sections:
                    with st.container(border=True):
                        ...
                        with col1:
                            ...
                            if st.button(f"Seleziona", key=f"select_section_{section['step_section_id']}"):
                                st.session_state.selected_section_id = section['id']
                                st.rerun()
                        
                        with col2:
                            # Pulsanti per riordinare la sezione
                            st.button("↑", key=f"up_{section['step_section_id']}", 
                                     help="Sposta su", 
                                     on_click=update_section_order, 
                                     args=(section['step_section_id'], max(1, section['order']-1)))
                            
                            st.button("↓", key=f"down_{section['step_section_id']}", 
                                     help="Sposta giù", 
                                     on_click=update_section_order,
                        ...
                        with col3:
                            ...
                            st.button("🗑️", key=f"delete_{section['step_section_id']}", 
                                     help="Elimina sezione", 
                                     on_click=delete_section_from_step,
                                     args=(section['step_section_id'],))
            else:
                st.info("Nessuna sezione configurata per questo step. Aggiungi una sezione.")
        else:
    ...
    with tab3:
        ...
        if 'selected_section_id' in st.session_state and st.session_state.selected_section_id:
            # Recupera il tipo di sezione
            section_type = next((s['sectiontype'] for s in all_sections if s['id'] == st.session_state.selected_section_id), "Sezione")
            ...
            with st.expander("Aggiungi nuovo componente"):
                st.text_input("Tipo di componente", key="new_component_type", 
            ...
            component_options = [component['component_type'] for component in all_components]
            component_ids = [component['id'] for component in all_components]
            
            st.selectbox("Seleziona un componente da aggiungere:", 
                        options=component_options,
                        key="selected_component_display")
            
            # Mappa il nome del componente selezionato all'ID
            if 'selected_component_display' in st.session_state and st.session_state.selected_component_display:
                try:
                    index = component_options.index(st.session_state.selected_component_display)
            ...
            st.button("Aggiungi componente alla sezione", on_click=add_component_to_section)
            
            # Mostra i componenti già associati alla sezione
            st.subheader("Componenti configurati per questa sezione")
            section_components = load_components_for_section(st.session_state.selected_section_id)
                ...
                for component in section_components:
                    with st.container(border=True):
                        ...
                        if component['structure']:
                            with st.expander("Struttura JSON"):
                                ...
                                if json_key not in st.session_state:
                                    st.session_state[json_key] = json.dumps(component['structure'], indent=2)
                                ...
                                if st.button("Salva struttura", key=f"save_json_{component['component_section_id']}"):
                                    try:
                                        json_data = json.loads(json_str)
                                        update_structure_data(component['structure_id'], json_data)
                        ...
                        with st.expander("Configurazione CMS"):
                            ...
                            if cms_json_key not in st.session_state:
                                if cms_key and 'value' in cms_key:
                                    st.session_state[cms_json_key] = json.dumps(cms_key['value'], indent=2)
                                else:
                            ...
                            if st.button("Salva configurazione CMS", key=f"save_cms_{component['component_section_id']}"):
                                try:
                                    cms_data = json.loads(cms_json_str)
                                    save_cms_key(component['structure_component_section_id'], cms_data)
                                except json.JSONDecodeError:
                                    st.error("JSON non valido. Controlla la sintassi.")
                        ...
                        with col1:
                            ...
                            st.button("↑", key=f"up_comp_{component['component_section_id']}", 
                                    help="Sposta su", 
                                    on_click=update_component_order, 
                                    args=(component['component_section_id'], max(1, component['order']-1)))
                        
                        with col2:
                            st.button("↓", key=f"down_comp_{component['component_section_id']}", 
                                    help="Sposta giù", 
                                    on_click=update_component_order,
                                    args=(component['component_section_id'], component['order']+1))
                        
                        with col3:
                            # Pulsante per eliminare il componente dalla sezione
                            st.button("🗑️", key=f"delete_comp_{component['component_section_id']}", 
                                    help="Elimina componente", 
                                    on_click=delete_component_from_section,
                                    args=(component['component_section_id'],))
            else:
                st.info("Nessun componente configurato per questa sezione. Aggiungi un componente.")
        else:
            st.info("Seleziona prima una sezione dalla tab 'Sezioni' per gestire i componenti.")
    ...
    with tab4:
        ...
        if st.session_state.current_step_id:
            ...
            st.selectbox(
                "Font primario:",
                options=["System default", "Arial", "Roboto", "Montserrat", "Open Sans"],
                key="ui_primary_font"
            )
        ...
        if step_sections:
            ...
            for section in sorted(step_sections, key=lambda x: x['order']):
                with st.container(border=True):
                    ...
                    if section_components:
                        for component in sorted(section_components, key=lambda x: x['order']):
                        ...
                        st.caption("Nessun componente in questa sezione. Aggiungi componenti dalla tab 'Componenti'.")
        else:
            st.info("Nessuna sezione configurata per questo step. Aggiungi sezioni dalla tab 'Sezioni'.")
    else:
        st.info("Seleziona uno step per visualizzare l'anteprima.")
    
    st.caption("Nota: questa è una visualizzazione semplificata dell'interfaccia. Nel frontend reale, i componenti saranno renderizzati correttamente in base alla loro configurazione.")
