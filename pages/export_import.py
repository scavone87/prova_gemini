...
from utils.export_import import export_funnel_config, import_funnel_config, format_export_for_download
...
st.set_page_config(page_title="Esporta/Importa Funnel", 
                   page_icon="📤", 
                   layout="wide",
...
def generate_filename(funnel_name):
    ...
    if funnel_name is None:
        ...
        safe_name = "".join(c if c.isalnum() or c in "._- " else "_" for c in funnel_name)
...
def load_available_funnels():
    ...
    try:
        ...
        return handle_error(
            e, 
            "Errore nel recupero dei funnel disponibili",
            fallback_data=[]
        ).get('data', [])
    finally:
...
with tab1:
    ...
    if not funnels:
        ...
        funnel_options = [f"{f['name']} - {f['product_name']} (ID: {f['id']})" for f in funnels]
        ...
        if st.button("📤 Esporta Funnel"):
            with st.spinner("Esportazione del funnel in corso..."):
                ...
                if not export_result.get('error', True):
                    ...
                    st.success(f"Funnel esportato con successo! Clicca il pulsante qui sotto per scaricarlo.")
                    st.download_button(
                    ...
                    st.error(f"Errore durante l'esportazione: {export_result.get('message', 'Errore sconosciuto')}")
...
with tab2:
    ...
    uploaded_file = st.file_uploader("Carica un file di configurazione funnel (JSON)", type="json")
    ...
    if uploaded_file is not None:
        try:
            ...
            with st.expander("Anteprima dei dati importati", expanded=True):
                ...
                st.write(f"**Prodotto:** {product_info.get('name', 'N/A')} (Codice: {product_info.get('code', 'N/A')})")
            ...
            if st.button("📥 Importa Funnel"):
                with st.spinner("Importazione del funnel in corso..."):
                    ...
                    if not import_result.get('error', True):
                        st.success(f"{import_result.get('message', 'Importazione completata con successo')}")
                        st.write(f"Funnel ID: {import_result.get('funnel_id')}")
                        st.write(f"Step importati: {import_result.get('steps_imported')}")
                        st.write(f"Route importate: {import_result.get('routes_imported')}")
                        
                        # Aggiungi link per navigare al funnel importato
                        st.page_link("pages/steps_manager.py", label="Vai alla gestione degli step", icon="🔄")
                    else:
                        st.error(f"Errore durante l'importazione: {import_result.get('message', 'Errore sconosciuto')}")
        
        except json.JSONDecodeError:
            st.error("Il file caricato non è un JSON valido. Verifica il formato del file.")
        except Exception as e:
